    ...
    def selectAllStudents(self):
        try:
            ...
            for item in dataset:
                ...
                self.tableWidgetStudent.setItem(row, 1, QTableWidgetItem(str(self.code)))
                self.tableWidgetStudent.setItem(row, 2, QTableWidgetItem(str(self.name)))
                self.tableWidgetStudent.setItem(row, 3, QTableWidgetItem(str(self.age)))
            cursor.close()
            # self.conn.close()
        except Exception as e:
            traceback.print_exc()
            QMessageBox.critical(None, "Error", "An unexpected error has occurred: " + str(e))
